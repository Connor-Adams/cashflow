...
| **Expectation** | planned → posted → skipped / ignored / cancelled | PlannedEvent + Subscription folded into one table (`kind='planned'\|'subscription'`; `cadence` column for recurring) — shipped 2026-05-31; + Reimbursement (a loan owed to you is expected money movement: expected → received → waived mirrors planned → posted → cancelled) | *expected* money movement |
