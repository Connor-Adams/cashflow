...
export function BudgetStatusCard({ currency = 'CAD' }: Props) {
  ...
  if (!items || items.length === 0) {
    return (
      <BentoTile span={6} rows={1} label="Budget status" description="Track your spending against targets.">
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item) => {
          const pct = typeof item.percentUsed === 'number' && Number.isFinite(item.percentUsed)
            ? item.percentUsed
            : 0
