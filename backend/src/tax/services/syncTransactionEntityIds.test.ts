...
let syncTransactionEntityIds: typeof import('./syncTransactionEntityIds').syncTransactionEntityIds;

before(async () => {
  const models = await import('../../models');
  ({ sequelize, Account, Entity, Household, Transaction } = models);
  ({ syncTransactionEntityIds } = await import('./syncTransactionEntityIds'));
