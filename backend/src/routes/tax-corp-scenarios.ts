...
//
// CRUD + fork/compute/compare routes for corp tax scenarios (P8a Tasks 4-5).
// Mirrors `tax-personal-scenarios.ts` but dispatches to the corp resolver /
// compute layer and rejects entities whose `kind !== 'corp'`.
//
// All endpoints are mounted under `/api/tax/corp-scenarios` and require auth
// (the global `requireAuth` middleware at `/api` enforces this).
...
router.post('/', async (req, res, next) => {
  try {
    ...
    try {
      validateOverrideMap(overrides, 'corp');
    } catch (err) {
      res
        .status(400)
    ...
    const baseline = await ensureCorpBaselineScenario(entityId, year);
...
// GET /api/tax/corp-scenarios?entityId=&year= — list scenarios for an
...
// GET /api/tax/corp-scenarios/compare?ids=1,2,3 — diff payload for N
...
router.get('/compare', async (req, res, next) => {
  try {
    ...
    // partial-permission compare can't leak any rows. Also enforce corp-only.
    ...
    if (entities.some((e) => e.kind !== 'corp')) {
...
router.patch('/:id', async (req, res, next) => {
  try {
    ...
    if ('overrides' in req.body) {
      try {
        validateOverrideMap(req.body.overrides, 'corp');
      } catch (err) {
        res
          .status(400)
          .json({ error: 'invalid_overrides', message: (err as Error).message });
