...
const { recomputeTransactionAmounts } = require('../import/calculateShares');
const { serializeTransaction } = require('../util/serializeTransaction');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.dateFrom || req.query.dateTo) {
      where.date = {};
      if (req.query.dateFrom) where.date[Op.gte] = String(req.query.dateFrom);
      if (req.query.dateTo) where.date[Op.lte] = String(req.query.dateTo);
    }
