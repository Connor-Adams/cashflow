...
let buildBundle: typeof import('../sync/buildBundle').buildBundle;
let restoreBundle: typeof import('../sync/restoreBundle').restoreBundle;

before(async () => {
  const models = await import('./');
  ({ sequelize, Account, Entity, Household, Transaction } = models);
  ({ buildBundle } = await import('../sync/buildBundle'));
  ({ restoreBundle } = await import('../sync/restoreBundle'));
