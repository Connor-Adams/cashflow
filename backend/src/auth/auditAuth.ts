...
export async function requireAuditAuth(req: Request, res: Response, next: NextFunction) {
  try {
    // GET-only surface — reject any non-GET method.
    if (req.method !== 'GET') {
      res.status(405).json({ error: 'Audit tokens are read-only' });
      return;
    }
    const header = String(req.headers.authorization ?? '');
    const match = header.match(/^Bearer\s+(\S+)$/i);
    const plaintext = match?.[1] ?? '';
    // Cross-type refusal: only cfa_ format accepted.
    if (!plaintext || !isAuditTokenFormat(plaintext)) {
      res.status(401).json({ error: 'Invalid audit token' });
      return;
    }
    const token = await UserAuditToken.findOne({ where: { tokenHash: hashAuditToken(plaintext) } });
