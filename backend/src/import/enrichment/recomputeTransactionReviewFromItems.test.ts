...
import { recomputeTransactionReviewFromItems } from './recomputeTransactionReviewFromItems';
...
test('categorizeAndApply with no orderId derives touched orders and clears reviewFlag', async () => {
  const { categorizeAndApplyReceiptItems } = await import('../categorizeReceiptItems.js');
...
test('backfill clears pre-existing fully-categorized itemized txns and is idempotent', async () => {
  const { backfillItemReviewClears } = await import('../enrichmentBackfillScheduler.js');
