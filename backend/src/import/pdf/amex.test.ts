...
import { parseAmexHeader, amexParser } from './amex';
...
function reserveHeaderLines(): PdfLine[] {
  return [
...
function cobaltHeaderLines(): PdfLine[] {
  return [
    mkLine(' American Express® Cobalt™ Card     1-800-869-3016 (toll free)', 1, 790),
    mkLine(' CONNOR ADAMS                         XXXX XXXXX2 02002   May 1, 2026   May 31, 2026', 1, 689),
  ];
}

test('sniff — matches Reserve Card', () => {
  assert.equal(amexParser.sniff(reserveHeaderLines()), true);
});

test('sniff — matches Cobalt Card', () => {
  assert.equal(amexParser.sniff(cobaltHeaderLines()), true);
});

test('sniff — rejects non-Amex', () => {
  assert.equal(amexParser.sniff([mkLine('CIBC Costco Mastercard')]), false);
});

test('parseAmexHeader — Reserve: extracts account, period, holder, productLabel', () => {
  const hdr = parseAmexHeader(reserveHeaderLines());
  ...
  assert.equal(hdr.productLabel, 'American Express Aeroplan Reserve Card');
});

test('parseAmexHeader — Cobalt: extracts dynamic productLabel', () => {
  const hdr = parseAmexHeader(cobaltHeaderLines());
  assert.equal(hdr.accountSuffix, '202002');
  assert.equal(hdr.periodStart, '2026-05-01');
  assert.equal(hdr.periodEnd, '2026-05-31');
  assert.equal(hdr.productLabel, 'American Express Cobalt Card');
});
...
test('parse — European thousands+decimal in FX amount (1.234,56)', () => {
  const lines = [
    ...reserveHeaderLines(),
    mkLine('New Transactions for CONNOR ADAMS', 2, 560),
    mkLine('May 7   May 8   LUXURY HOTEL   PARIS', 2, 387),
    mkAmountLine('2026.84', 385),
    mkLine('EUROPEAN UNION EURO 1.234,56    @ 1.64105', 2, 378, 100),
    mkLine('Total of New Transactions for                                        2026.84', 2, 350),
  ];
  const result = amexParser.parse(lines, { defaultCurrency: 'CAD' });
  assert.equal(result.transactions.length, 1);
  assert.ok(
...
test('parse — Cobalt header returned with correct productLabel', () => {
  const lines = [
    ...cobaltHeaderLines(),
    mkLine('Total of Payment Activity   0.00', 2, 632),
  ];
  const result = amexParser.parse(lines, { defaultCurrency: 'CAD' });
  assert.ok(result.header);
  assert.equal(result.header!.productLabel, 'American Express Cobalt Card');
});
